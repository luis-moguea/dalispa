  ...
  return (
    <>
      ...
      <Box p="2em" mx="auto" px="4" mt="6em">
        <Heading textAlign="center">TRADICIONAL</Heading>
        <Grid
          objectFit="cover"
          templateColumns={
            tradiData.length <= 2 ? "repeat(2, 1fr)" : "repeat(2, 1fr)" // 2 columnas si hay 2 imágenes, 3 si hay más
          }
          templateRows={
            tradiData.length <= 2 ? "repeat(1, 400px)" : "repeat(4, 200px)" // 1 fila si hay 2 imágenes, 2 filas si hay más, ajustando la altura a 150px
          }
          gap={15} // Ajuste del espacio entre las imágenes
          p="2em"
          autoRows="150px" // Altura automática de las filas
        >
          {renderImages(tradiData)}
        </Grid>
      </Box>
    </>
  );
